...
アダプタ群だけ**」。use case 本体・実行手順は共通。
  ...
  Repository     | `DuckDbAssetRepository`         | `_PrintAssetRepository`（`--write` で DuckDB）
...
公開面は広げない）。
  ...
  --dry-run（既定）: Repository を print 専用 fake に差し替え、実データストアを汚さず
                     抽出結果を目視確認する。
  --write          : 実データストア（DuckDB + Parquet）へ保存する。
...
ローカル JSON（既定 ./collect.local.json, *.local.json は gitignore 済み）の形:
  {
    ...
    // --dry-run では未使用。--write 時のみ必要。ローカル Parquet パス（例
    // "./data.local.parquet"）または "s3://bucket/key.parquet"（実行環境の AWS 認証を使用）。
...
def build_use_case(
    ...
    *,
    ...
    本番 `handler_collect.build_use_case`（SSM → 版ピン chrome / DuckDB / S3）と対をなす。
    ...
    if write:
        ...
        print(f"[write] 実データストアへ保存します: {config['data_location']}")
    else:
        repository = _PrintAssetRepository()
        print("[dry-run] 実データストアには書き込みません")
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        "--dry-run",
        action="store_true",
        help="実データストアを汚さず print 専用リポジトリで結果を表示（既定）",
    )
    mode.add_argument(
        "--write",
        action="store_true",
        help="実データストア（DuckDB + Parquet）へ保存する",
    )
    parser.add_argument(
        "--config",
        type=Path,

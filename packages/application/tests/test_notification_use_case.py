...
def test_non_positive_days_raises_and_does_not_send(
    days: int,
    repository: InMemoryAssetRepository,
    notifier: RecordingNotifier,
    clock: FixedClock,
) -> None:
    # days<1 は未来窓となり 0件 skip と区別できないため、設定不備として弾く。
    repository.save(_portfolio(date(2026, 6, 18), 100_000, 20_000, 120_000))
    use_case = NotifySummaryUseCase(repository, notifier, clock)

    with pytest.raises(ValueError):
        use_case.execute(days)
    assert notifier.sent == []

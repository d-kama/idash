...
| 決定記録 | アクセス制御方式は **ADR-0006** を起こす（public repo 制約・CFn SecureString 非対応・synth 認証不要の3制約が背景） |
...
- `common.settings` に **`BffSettings`**（`env_name` / `data_location`）を追加（既存
  `NotifySettings` と同形式）。SSM 機密は不要（データストアは実行ロール認証）。
...
- ドキュメント（ADR-0006 / PROJECT_PLAN.md 更新 / README に KVS 投入・デプロイ手順）
...
- [ ] 11. **IdashFrontendStack**: `basic-auth.js`（JS 2.0 + KVS）+ `KeyValueStore` +
        ...
        BucketDeployment。`bin/app.ts` 配線、Taskfile `build-front`・`synth` deps 更新、
        スナップショット（asset ハッシュ正規化）。`task check` + `task synth` 緑。
- [ ] 12. **ドキュメント**: ADR-0006（ダッシュボードのアクセス制御 = CF Functions Basic 認証 +
        KVS。背景3制約と代替案比較）/ PROJECT_PLAN.md の Phase 5/6 TODO 解消・SPA
        フォールバック方針の更新 / README に KVS 手動投入手順とデプロイ順序。
- [ ] 13. **Phase 6 実デプロイ・E2E 確認**: KVS へ資格情報投入 → `task deploy`（--all）→
        CloudFront URL で Basic 認証 → ダッシュボード表示・期間セレクタ・指標トグル・
        実データ描画を確認。issue #27 クローズ。
...
- **Phase 5 完了〜Phase 6 完了の間、API GW 直 URL が認証なしで存在**: 自動命名の推測困難 URL
  のみで保護（obscurity）。期間を短くする。恒久対応（CloudFront 経由限定化、例: カスタム
  ヘッダ検証）は必要になったら検討。

...
5. **AWS Budgets アラート＋ Cost Anomaly Detection** ← 無料
   - 月額閾値（例: $1 / $5）でメール通知。普段ほぼ $0 のため少額異常でも即浮く。

### 不採用（確定）

- **AWS WAF**: レートベースルールは強力だが固定費（ルール月額＋リクエスト課金）が発生するため**使わない**。上記1〜5の無料セットで代替し、必要を感じた時点で再検討する。
...
| Budgets / Anomaly Detection | 任意スタック（または手動設定）。**TODO: CDK 化するか手動か** |

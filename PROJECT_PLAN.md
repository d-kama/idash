...
- **依存（禁止）**: フレームワーク・ランタイム（**Lambda / boto3 / FastAPI / Mangum**）、および具象実装（データストア / 外部サイト接続 / 通知の実体）。
...
`infrastructure` はデータストア（DuckDB+S3）だけでなく外部サイトクライアント・通知の具象も内包するため、実態は「外部システムとのアダプタ全般＝**インフラ層**」。Phase 0 で `repository` から `infrastructure` へ改称済み。当面は広めに使い、肥大化したらサブモジュールへ分割する（**TODO: 分割要否は実装後に判断**）。
...
- **パラメータの用途別分離**（命名規約は実装時確定、例: `/idash/<env>/...`）:
  - ① 外部DC年金サイトのログイン情報（`source-login`）
  - ② 通知チャネル（LINE）の認証情報（`notify-line`）
  - データストア（DuckDB + S3 Parquet）は実行ロール認証（credential_chain）のため SSM の機密は持たない（ADR-0005）
...
- **CDK での参照（確定）**: SecureString は CDK/CloudFormation では作成しない（作成済みを運用）。CDK は `ssm.StringParameter.fromSecureStringParameterAttributes` で**パラメータ名からインポート**し、`grantRead` で IAM 付与。Lambda には取得したリソースの **ARN（`parameterArn`）** を環境変数で渡す（例: `SOURCE_LOGIN_PARAM_ARN` / `NOTIFY_LINE_PARAM_ARN`）。実行時はその ARN を `GetParameter` の `Name` に使用。
- **TODO**:
...
│   ├── infrastructure/         # インフラ層: domain のポートを実装（DuckDB+S3 / 外部サイト接続 / 通知の具象）
...
- `infrastructure` は `domain` のポートを実装する具象（DuckDB+S3 / 外部サイト / 通知）。
...
| **ECR（コンテナイメージ保管）** | **$0.12〜0.30** | $0.10/GB・月。bff/batch の2アセット × 2世代＝4イメージ保持で計1.2〜3GB。**唯一の実費**。保持数を絞りすぎると稼働中 Lambda のイメージが消えるため下限は4（README「事前: bootstrap ECR のライフサイクル」参照） |

### 合計

**約 $0.15〜0.7 / 月（≒ 月20〜105円程度）**。実費はほぼ ECR 保管料のみ。

### 想定外課金の注意点

- **NAT Gateway を発生させない**: Lambda を VPC に入れると NAT Gateway が約 **$32/月**＋通信量で跳ねる。**本構成は Lambda を VPC に入れない**（S3 も外部サイトもインターネット/AWS エンドポイント経由のため VPC 不要）。← 最重要
